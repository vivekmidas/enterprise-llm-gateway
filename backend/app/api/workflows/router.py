...
async def get_workflows():
    logger.debug("get_workflows_request")
    workflows = await list_workflows_from_store()
    logger.debug("get_workflows_response", count=len(workflows), workflows=workflows)
    return workflows


@router.get("/{workflow_id}", response_model=WorkflowDefinition)
async def get_workflow(workflow_id: str, version: Optional[str] = None):
    logger.debug("get_workflow_request", params={"workflow_id": workflow_id, "version": version})
    try:
        workflow = await load_workflow_from_store(workflow_id, version)
        logger.debug("get_workflow_response", workflow_id=workflow_id, data=workflow.model_dump())
        return workflow
...
async def create_workflow(request: WorkflowSaveRequest):
    logger.debug("create_workflow_request", input_data=request.model_dump())
    ...
    try:
        ...
        logger.debug("create_workflow_response", result=saved_workflow)
...
async def remove_workflow(workflow_id: str, version: Optional[str] = None):
    logger.debug("remove_workflow_request", workflow_id=workflow_id, version=version)
    ...
    logger.debug("remove_workflow_response", workflow_id=workflow_id, version=version, status="success")
