...
def _refresh_customers_table(sync_conn):
    ...
    if "plugin_storage_path" not in columns:
        sync_conn.exec_driver_sql("ALTER TABLE customers ADD COLUMN plugin_storage_path VARCHAR")
    if "email" not in columns:
        sync_conn.exec_driver_sql("ALTER TABLE customers ADD COLUMN email VARCHAR")
    if "address" not in columns:
        sync_conn.exec_driver_sql("ALTER TABLE customers ADD COLUMN address VARCHAR")
...
def _refresh_nodes_table(sync_conn):
    ...
    if "customer_id" not in columns:
        sync_conn.exec_driver_sql("ALTER TABLE nodes ADD COLUMN customer_id INTEGER")
